...
class Cart extends Component {
	...
	deleteCart = (pCart) => {
		axios.delete(`${backendUrl}/carts/${pCart.id}`).then((response) => {
			this.setState({
	...
	deleteCartDetail = (pCartDet) => {
		axios.delete(`${backendUrl}/cartdets/${pCartDet.id}`).then((response) => {
		...
		if (currentCart) {
			infoCart = currentCart.map((cart) => {
				const cartDetails = cart.CartDetails.map((detail, dIndex) => {
					const bookDet = (
						<div key={detail.Book.id} className="book-det">
							<div className="book-det-item">
								{dIndex + 1}
								{".-  "}
							</div>
							<img
								className="book-det-item cartImage"
								src={`../booksImages/${detail.Book.Img}`}
								alt="Boook"
							></img>
							<div className="book-det-item book-title-cart">
				...
				return (
					<div key={cart.id} className="cart-content">
						<div>
						...
						<div className="cart-delivery">
							...
							<div className="cart-delivery-dates">
								<div className="cart-dd-item">
									...
									{cart.PurchaseDate.replace(/[A-Za-z]/g, " ")}
								</div>
								<div className="cart-dd-item">
									<strong>DeliveryDate:</strong>{" "}
									{cart.DeliveryDate
										? cart.DeliveryDate.replace(/[A-Za-z]/g, " ")
										: null}
								</div>
								<div className="cart-dd-item">
									<strong>Total:</strong>
			...
			<div className="cart-container-above">
				<div className="cart-container-screen">
					...
					{currentUser && (!currentCart || currentCart.length <= 0) && (
						<div className="cart-message">
							<h1>
								<strong>{"There isn't a cart open."}</strong>
							</h1>
						</div>
					)}
				</div>
